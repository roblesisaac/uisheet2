...
Chain.prototype._library = {
  ...
  steps: {
    "&": function(last) {
      if(!this._conditions) {
        this._addTools({ _conditions: [last] });
      }
    },
    ...
    each: function(notSure, next) {
      ...
      getData.method(this, (data) => {
    		...
    		for(var i=0; i<data.length; i++) {
    			loop.method(memory({ i, item: data[i] }));
    		}
  ...
  var chainMethod = function(memory) {
    ...
    var getMemory = (_res, _rej, _chainName) => {
      return memory && memory._remember
        ? memory
        : new Memory(chain)._addTools({ _res, _rej, _chainName, _prints });
    }
...
function getStep(sIndex) {
  ...
  if(!stepPrint) return { chainIsMissing: sIndex };
  
  for(var i=0; i<sIndex; i++) {
    stepPrint = stepPrint.nextStep || { indexNotFound: sIndex };
  }
  ...
  var buildSub = function(index, sProp, instructs, previous) {
	...
	return {
		...
		method: function(memory, rabbitTrail) {
          ...
          { _rej, _res, _prints, _print, _chainName } = memory;
      ...
      if(variation) {
    	...
    	try {
    	  ...
    	  handleError({ error, methodName, chainName, _chainName, currentPrint });
    	  return;
    	}
